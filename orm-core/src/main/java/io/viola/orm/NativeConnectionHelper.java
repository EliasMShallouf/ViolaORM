...
import io.viola.orm.columns.OrderColumn;
import io.viola.orm.columns.SetColumn;
import io.viola.orm.exceptions.PrimaryKeyNotFoundException;
import io.viola.orm.helpers.LogicalStream;
import io.viola.orm.helpers.StringHelper;
...
import io.viola.orm.query.QueryBuilder;
...
public class NativeConnectionHelper {
    ...
    public static <E> E createFromResultSet2(ResultSet resultSet, Class<E> clazz)
            throws SQLException {
        if(ClassHelper.isPrimitive(clazz)) {
            ResultSetMetaData metaData = resultSet.getMetaData();
            return (E) value(metaData.getColumnName(1), clazz, resultSet);
        ...
        ClassGraph<E> graph = ReflectionCaching.getCache().getGraph(clazz);

        graph.fields.forEach((f, gf) -> {
            try {
                Field rf = clazz.getDeclaredField(gf.getFieldName());
                rf.setAccessible(true);

                if (gf.isLob())
                    rf.set(e, blob(gf.getColumnName(), gf.getClazz(), resultSet));
                else
                    rf.set(e, value(gf.getColumnName(), gf.getClazz(), resultSet));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        });
    ...
    public static <E> E fetchOne(ResultSet resultSet, Class<E> clazz)
            throws SQLException {
        if(resultSet.next()) {
            return createFromResultSet2(resultSet, clazz);
        }
        return null;
    }
    ...
    public static <E> List<E> fetch(ResultSet resultSet, Class<E> clazz)
        ...
        while (resultSet.next()) {
            list.add(createFromResultSet2(resultSet, clazz));
        }
    ...
    public static <E, Id> int updateById(Connection connection, EntityModel<E, Id> model, E e, Id id) {
        try {
            ...
            for (Field f : clazz.getDeclaredFields()) {
                if (
                    f.isAnnotationPresent(io.viola.orm.annotations.Id.class) ||
                    f.isAnnotationPresent(Column.class) ||
                    f.isAnnotationPresent(Lob.class)
                ) {
                    String fieldName = LogicalStream
